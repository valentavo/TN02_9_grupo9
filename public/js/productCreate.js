  ...
  createProduct.addEventListener('click', async () => {
    ...
    const variantCombinations = [
      {
        ...
        'colores-fk': +productColor.value > 0 ? +productColor.value : null,
        'medidas-fk': +productSize.value > 0 ? +productSize.value : null,
      },
    ];
    ...
    if (errors.length == 0 && !multipleCombination) {
      ...
      if (resFetch.meta.success) {
        ...
        window.location.href = `/product/detail/${resFetch.data.id}-${resFetch.data.product[0].id}`;
      } else {
        await Swal.fire({
          icon: 'error',
          title: 'Ups!',
