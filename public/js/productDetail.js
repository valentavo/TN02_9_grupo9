...
const availableCombination = (sizeValue) => {
    ...
    if (colorSelect) {
        ...
        allStockCombinations.filter(product => (product.size.id == sizeValue)).forEach(product => {
...
async function ready() {
    ...
    */
    ...
    if(productGroup.product.find(row => row['medidas-fk'] != null)) {
        ...
        const sizeList = [...new Set(productGroup.product.map(prod => prod.size.id))].map(id => {
            return {
                id: id,
                medida: productGroup.product.find(prod => prod.size.id == id).size.medida
            }
        });

        sizeList.forEach( size => {
            sizeOptions += `<option value='${ size.id }' ${productDetail['medidas-fk'] == size.id ? 'selected': ''} ${allStockCombinations.filter(product => (product.size.id == size.id)).length == 0 ? 'disabled' : ''}>${ size.medida }</option>`;
        });
    ...
    if(productsWtihColor.length > 0) {
        ...
        let colorsVariant = allStockCombinations.filter(product => product.size.id == sizeSelect.value);
        ...
        if(colorsVariant.length == 0) {
            ...
            colorsVariant = productGroup.product.map(product => {
                return {
        ...
        if(colorsVariant[0].color) {
            ...
            colorsVariant.forEach( prod => {
                colorOptions += `<option class="colorOptions" id="color-${prod.color.id}" value='${ prod.color.id }' ${productDetail['colores-fk'] == prod.color.id ? 'selected': ''}>${ prod.color.nombre }</option>`;
            });
        };
