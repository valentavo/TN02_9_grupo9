    ...
    return (productsContainer.innerHTML += `
        <div class="card col mb-4 col-lg-4 col-md-12 col-sm-12 col-12">
            ...
            <div class="card-body">
                ...
                <a class="btn my-4 py-3 px-3 productCartBtn h-auto" href="/product/${
                  userRol == '2' ? 'edit' : 'detail'
                }/${productGroup.id}-${productGroup.product[0].id}"> ${
      userRol == '2' ? 'Editar producto' : 'Detalle'
    } </a>
            </div>
        </div>
        `);
...
const productsFilter = (type, name, element) => {
  const filter =
    (type[0] == 'categorias-fk' && selectedCategory == 0) || (type[0] == 'marcas-fk' && selectedBrand == 0);
  ...
  if (element.id !== 'shopAllbtn') {
    ...
    if (
      (type[0] == 'categorias-fk' && element.id !== 'category-all') ||
      (type[0] == 'marcas-fk' && element.id !== 'brand-all')
    )
      element.classList.add('unavailable-option');
  } else {
    productsFiltered = allProducts;
...
async function ready() {
  ...
  products.data.categories.forEach((category, y) => {
    categoryList.innerHTML += `<li><button  type="button" id="category-${y}" class="category-filter-option" onClick="productsFilter(['categorias-fk', ${category.id}], '${category.nombre}', this)" >${category.nombre}</button></li>
        `;
  });

  //Marcas
  products.data.brands.forEach((brand, y) => {
    return (brandList.innerHTML += `<li><button type="button" id="brand-${y}" class="brand-filter-option" onClick="productsFilter(['marcas-fk', ${brand.id}], '${brand.nombre}', this)" >${brand.nombre}</button></li>
        `);
  });
